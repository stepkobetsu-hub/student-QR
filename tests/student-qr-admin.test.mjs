...
test('selected QR cards can be printed together', () => {
  ...
  assert.match(page, /body\.batch-print #batchPrintHost/);
  ...
  assert.match(page, /id="batchPrintHost" aria-hidden="true"/);
  assert.match(page, /body\.batch-print > :not\(#batchPrintHost\)/);
  assert.match(page, /body\.batch-print #batchPrintHost \{[\s\S]*?position: static !important/);
  assert.match(page, /batchPrintHostEl\.innerHTML = checkBatchPrintAreaEl\.innerHTML/);
  assert.match(page, /document\.getElementById\('batchPrintHost'\)\.innerHTML = ''/);
