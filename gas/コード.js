...
function doPost(e) {
  ...
  try {
    ...
    if (/^teacherRegistration/.test(action)) {
      result = handleTeacherRegistration_(body);
    } else if (isPointManagerRequest && !pointManagerAvailable) {
      result = {
        ok: false,
...
function saveStudentAttendance_(values, receiptId, trace, hasNotificationTargets, acceptedDate, trustedAttendanceType) {
  ...
  // 60秒判定の正本は「入退室ログ2」の最新保存行とする。
  const latestSavedAttendance = getLatestStudentAttendanceLog_(logSheet, code);
  const duplicateBaseStampMs = latestSavedAttendance ? latestSavedAttendance.stampMs : state.lastStampMs;

  if (isWithinCheckInDuplicateWindow_(duplicateBaseStampMs, now.getTime())) {
    ...
    return {
      ...
      name: name,
      school: school,
      type: latestSavedAttendance.type || state.lastType || (state.count % 2 === 0 ? '退室' : '入室'),
      label: Utilities.formatDate(new Date(duplicateBaseStampMs), 'Asia/Tokyo', 'M月d日H時mm分'),
      totalPoints: Number(state.totalPoints || 0),
      logRow: Number(latestSavedAttendance.logRow || state.lastLogRow || 0),
